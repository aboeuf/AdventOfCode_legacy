...
public:
    ...
    RangeMap(const QString& input) {
        ...
        m_source_range = Range{source_range_start, source_range_start + range_length - 1u};
        m_destination_range = Range{destination_range_start, destination_range_start + range_length - 1u};
    }
...
public:
    ...
    void mergeInsert(const Range& range) {
        ...
        if (stop_it == m_ranges.end()) {
            for (auto it = m_ranges.begin(); stop_it != m_ranges.end() and it != m_ranges.end(); ++it) {
...
private:
    Int getNextDestinationIndex(const QString& source, const Int source_index, QString& destination) const {
    ...
    Int getSeedLocation(const Int index) const {
        ...
        while (current_map != destination_map and m_maps.contains(current_map)) {
            current_index = getNextDestinationIndex(current_map, current_index, next_map);
    ...
    void getNextRanges(QString& current_map, SortedRanges& current_ranges) const
    {
        ...
        for (const auto& range : current_ranges.ranges()) {
            const auto mapped_start = m_maps[current_map].getDestinationIndex(range.start());
            const auto mapped_stop = m_maps[current_map].getDestinationIndex(range.stop());
