...
class Helper {
public:
    static const std::vector<QChar>& labels() {
        return m_index_to_label;
    }

    static Int nbLabels() {
        return m_index_to_label.size();
    }

    static QChar getLabel(Int index) {
        if (index >= Helper::nbLabels())
            common::throwInvalidArgumentError(QString("Helper::getLabel: out-of-range index %1 (nbLabels = )").arg(index).arg(Helper::nbLabels()));
    ...
    static Int getIndex(const QChar& label, bool v1) {
        if (not isValidLabel(label))
            common::throwInvalidArgumentError(QString("Helper::getIndex: label '%1' is not valid").arg(label));
        if (v1 or label != 'J')
...
std::vector<QChar> Helper::m_index_to_label = std::vector<QChar>{'2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'};
...
Type getType(const QString& hand, bool v1)
{
    if (v1 or not hand.contains('J')) {
        ...
        common::throwInvalidArgumentError(QString("getType: bad occurences \"%1\" for hand \"%2\"")
    ...
    if (highest_type == Type::None)
        common::throwRunTimeError(QString("getType: failed to determine highest type for hand \"%1\"").arg(hand));
    return highest_type;
}
...
public:
    Hand(const QString& input, bool v1) : m_v1{v1} {
        ...
        if (splitted.size() != 2) {
            ...
            dbg << splitted;
            common::throwInvalidArgumentError(QString("Hand::constructor: bad split %1").arg(splitted_str));
        }
        m_hand = splitted[0];
        if (m_hand.size() != 5)
            common::throwInvalidArgumentError(QString("Hand::constructor: invalid hand size \"%1\"").arg(m_hand));
        for (const auto& c : m_hand)
            if (not Helper::isValidLabel(c))
                common::throwInvalidArgumentError(QString("Hand::constructor: label '%1' is not valid").arg(c));
        bool ok;
        m_bid = splitted[1].toUInt(&ok);
        if (not ok)
            common::throwInvalidArgumentError(QString("Hand::constructor: cannot convert \"%1\" to uint").arg(splitted[1]));
