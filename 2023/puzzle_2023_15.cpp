...
struct Lens
{
    ...
    Lens(const QString& input) {
        ...
        if (splitted.size() != 2)
            common::throwInvalidArgumentError(QString("Lens::constructor: cannot parse \"%1\"").arg(input));
        label = splitted[0];
        auto ok = true;
        focal_length = splitted[1].toUInt(&ok);
        if (not ok)
            common::throwInvalidArgumentError(QString("Lens::constructor: cannot convert \"%1\" to unsigned int").arg(splitted[1]));
    }

    uint focusingPower(const uint box_multipler, const uint slot_multiplier) const {
    ...
    LavaProductionFacility(const QString& input) {
        ...
        for (const auto& instruction : instructions) {
            if (instruction.isEmpty())
                common::throwInvalidArgumentError("LavaProductionFacility::constructor: empty instruction");
            if (instruction.back() == '-') {
