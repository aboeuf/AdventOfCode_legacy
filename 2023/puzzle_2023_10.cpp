...
{
public:
    Cell(const CellIndex& max, const QChar& label, int x, int y) : m_label{label}, m_index{x, y}, m_neighbors{}
    {
        if (x > 0 and (m_label == 'S' or m_label == '|' or m_label == 'L' or m_label == 'J'))
            m_neighbors.insert(CellIndex{x - 1, y});
        if (x + 1 < max.x and (m_label == 'S' or m_label == '|' or m_label == '7' or m_label == 'F'))
            m_neighbors.insert(CellIndex{x + 1, y});
        if (y > 0 and (m_label == 'S' or m_label == '-' or m_label == 'J' or m_label == '7'))
            m_neighbors.insert(CellIndex{x, y - 1});
        if (y + 1 < max.y and (m_label == 'S' or m_label == '-' or m_label == 'L' or m_label == 'F'))
            m_neighbors.insert(CellIndex{x, y + 1});
    }
    ...
    void pruneNeighbors(const std::function<const std::set<CellIndex>&(const CellIndex&)>& get_neighbors) {
...
struct Probe : public QList<CellIndex>
{
    ...
    Probe(const CellIndex& start) : QList<CellIndex>{} {
        *this << start;
    }
    ...
    CellIndex current() const {
        return empty() ? CellIndex{-1, -1} : back();
    }
    ...
    QString toString() const
    {
        ...
        return QString("current: %1 | previous: %2 | full: %3")
                .arg(current().toString()).arg(previous().toString()).arg(full);
    }
};
...
public:
    Map(const QString& input) {
        ...
        const auto get_neighbors = [this](const CellIndex& index) -> const std::set<CellIndex>& {
    ...
    {
        auto area = 0;
        for (auto it = std::begin(m_loop); std::next(it) != std::end(m_loop); ++it) {
    ...
    QString solveOne() const {
        return QString("%1").arg(perimeter() / 2);
    }
                ...
                for (const auto& neighbor : cell.neighbors()) {
                    if (neighbor != current_probe.previous()) {
                        ...
                        if (not insert_result.second) {
                            m_loop.reserve(insert_result.first->size() + next_probe.size() - 2);
                            for (auto& index : *insert_result.first)
                                m_loop << index;
                            for (auto it = std::next(std::rbegin(next_probe)); it != std::rend(next_probe); ++it)
                                m_loop << *it;
                            return;
                        }
                    }
