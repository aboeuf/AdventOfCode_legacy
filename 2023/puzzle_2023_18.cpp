...
inline Direction charToDirV1(const QChar& c)
{
    ...
    common::throwInvalidArgumentError(QString("charToDirV1: unknown direction '%1'").arg(c));
    return Direction::up;
}
...
inline Direction charToDirV2(const QChar& c)
{
    ...
    if (c == '3')
        return Direction::up;
    common::throwInvalidArgumentError(QString("charToDirV2: unknown direction '%1'").arg(c));
    return Direction::up;
}
...
struct Point
{
    ...
    Point(const Point& start, Direction direction, UInt lenght) : x{start.x}, y{start.y} {
        if (direction == Direction::up)
            ...
            common::throwInvalidArgumentError("Point::constructor: unknown direction");
    }
...
public:
    Edge(const QString& input, const Point& start, bool v1) {
        ...
        if (splitted.size() != 3)
            common::throwInvalidArgumentError(QString("Edge::constructor: wrong input size (%1). Should be (3).").arg(splitted.size()));
        if (v1) {
            if (splitted[0].size() != 1)
                common::throwInvalidArgumentError(QString("Edge::constructor: wrong direction size (%1). Should be (1).").arg(splitted[0].size()));
            m_direction = charToDirV1(splitted[0].front());
            auto ok = true;
            m_length = static_cast<UInt>(splitted[1].toUInt(&ok));
            if (not ok)
                common::throwInvalidArgumentError(QString("Edge::constructor: cannot cast \"%1\" to uint").arg(splitted[1]));
        } else {
            if (splitted[2].size() != 9)
                common::throwInvalidArgumentError(QString("Edge::constructor: wrong color size (%1). Should be (9).").arg(splitted[2].size()));
            auto ok = true;
            m_length = static_cast<UInt>(splitted[2].mid(2, 5).toUInt(&ok, 16));
            if (not ok)
                common::throwInvalidArgumentError(QString("Edge::constructor: cannot cast hexadecimal \"%1\" to uint").arg(splitted[2].mid(2, 5)));
...
public:
    ...
    UInt area() const {
        ...
        for (const auto& edge : m_edges)
            signed_area += (edge.start().x * edge.stop().y - edge.stop().x * edge.start().y);
        return 1u + (static_cast<UInt>(std::abs(signed_area)) + perimeter()) / 2u;
    }

    QString solve() const {
        return QString("%1").arg(area());
    }
