...
const auto directions = QVector<Direction>{Direction::north, Direction::west, Direction::south, Direction::east};
...
struct Vertex
{
    ...
    Vertex(const Vertex& previous, const Direction next) : Vertex{previous}
    {
        if (next == Direction::north)
            ...
            common::throwInvalidArgumentError("Vertex::constructor: unknown direction");
    }
...
uint qHash(const Vertex& vertex) {
...
public:
    Grid(const QString& input) : m_start{-1, -1}, m_end{-1, -1} {
        ...
        if (m_start.x < 0 or m_start.y < 0)
            common::throwInvalidArgumentError("Grid::constructor: cannot find start point");
        ...
        if (m_end.x < 0 or m_end.y < 0)
            common::throwInvalidArgumentError("Grid::constructor: cannot find end point");
    }
    ...
    bool isFree(const Vertex& vertex) const {
        return vertex.x > -1 and vertex.x < m_length and vertex.y > -1 and vertex.y < m_width and m_cells[vertex.x][vertex.y] != QChar('#');
    }
...
public:
    Graph(const QString& input, bool is_icy) : m_grid{input} {
        ...
        for (auto i = 0; i < m_grid.length(); ++i) {
            for (auto j = 0; j < m_grid.width(); ++j) {
                ...
                if (m_grid.isFree(head)) {
                    for (const auto direction : getNextDirections(m_grid.cells()[i][j], is_icy)) {
        ...
        exportToCSV("/home/aboeuf/Projects/AdventOfCode/python/2023/day_23/before.txt");

        const auto get_prunning_neighbors = [this](const QHash<Vertex, Neighbors>::iterator& vertex) -> QSet<Vertex> {
        ...
        for (; vertex != std::end(m_vertices);) {
            ...
            if (neighbors.size() == 2) {
                ...
                if (A_to_vertex == std::end(m_edges))
                    common::throwRunTimeError("Graph::constructor: cannot find edge A to vertex");

                const auto vertex_to_A = m_edges.find(Edge{vertex.key(), A.key()});
                if (vertex_to_A == std::end(m_edges))
                    common::throwRunTimeError("Graph::constructor: cannot find edge vertex to A");

                const auto B_to_vertex = m_edges.find(Edge{B.key(), vertex.key()});
                if (B_to_vertex == std::end(m_edges))
                    common::throwRunTimeError("Graph::constructor: cannot find edge B to vertex");

                const auto vertex_to_B = m_edges.find(Edge{vertex.key(), B.key()});
                if (vertex_to_B == std::end(m_edges))
                    common::throwRunTimeError("Graph::constructor: cannot find edge vertex to B");
        ...
        exportToCSV("/home/aboeuf/Projects/AdventOfCode/python/2023/day_23/after.txt");
    }
    ...
    QString solve() const {
        ...
        while (not Q.empty()) {
            ...
            if (current.last == m_grid.end()) {
                ...
                for (auto child = std::begin(parent->children); child != std::end(parent->children); ++child) {
    ...
    void exportToCSV(const QString& filepath) const {
        ...
        if (not file.open(QIODevice::WriteOnly | QIODevice::Text))
            common::throwInvalidArgumentError(QString("Graph::exportToCSV: cannot open file \"%1\"").arg(filepath));
