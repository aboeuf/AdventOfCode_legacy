...
public:
    ...
    uint inputBeamAt(int x_init, int y_init, Direction direction_init) const {
        ...
        const auto enqueue = [&queue, &map, this](int x, int y, Direction direction) {
            ...
            next_state.changed_state = map[next_state.coordinates].inputBeam(m_tiles[x][y], direction);
    ...
    QString solveTwo() {
        ...
        for (auto y = 0; y < m_length; ++y) {
            ...
            maximum = std::max(maximum, inputBeamAt(m_length - 1, y, Direction::north));
        }
