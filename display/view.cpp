...
View::View(QWidget* parent) : QGraphicsView{new QGraphicsScene{-1.e4, -1.e4, 2.e4, 2.e4}, parent}
{
...
void View::mouseMoveEvent(QMouseEvent* event)
{
  if (m_drag_mode_on) {
    ...
    horizontalScrollBar()->setValue(horizontalScrollBar()->value() + delta_viewport.x());
    verticalScrollBar()->setValue(verticalScrollBar()->value() + delta_viewport.y());
  }
...
void View::wheelEvent(QWheelEvent* event)
{
  ...
  if (angle != 0) {
    ...
    const auto delta_viewport_pos =
        m_viewport_pos - QPointF{viewport()->width() / 2.0, viewport()->height() / 2.0};
