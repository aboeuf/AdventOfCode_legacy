...
public:
    PrioritiesComputeur()
    {
        const auto items = QString("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
...
public:
    Rucksack(const QString& input) : m_contents{input}
    {
        ...
        if (first_compartment.size() != second_compartment.size() or first_compartment.isEmpty())
            return;
...
public:
    ...
    bool isValid() const
    {
        return m_sacks.size() % 3 == 0 and
                std::all_of(std::begin(m_sacks), std::end(m_sacks), [](const auto& sack) {
            return sack.isValid();
        });
    }
...
void Solver_2022_03_1::solve(const QString& input)
{
    ...
    emit output(QString("Input is %1").arg(sacks.isValid() ? "valid" : "invalid"));
