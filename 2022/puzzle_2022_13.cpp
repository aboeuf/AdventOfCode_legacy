...
struct Packet
{
    Packet(const QString& data) : str{data} {
        ...
        if (data.startsWith('[') and data.endsWith(']')) {
            ...
            for (auto end = 1; end < data.size() - 1; ++end) {
                if (data[end] == ',' and counter == 0u) {
                    ...
                    if (counter == 0u)
                        throw std::invalid_argument(QString("Packet: ill formed data \"%1\"")
                                                    .arg(data).toStdString());
                    --counter;
                }
            }
        ...
        if (not ok)
            throw std::invalid_argument(QString("Packet: cannot convert \"%1\" to unsigned int")
                                        .arg(data).toStdString());
    }
...
inline Order compare(const Packet& left, const Packet& right)
{
    ...
    for (auto i = 0u; i < std::min(left.contents.size(), right.contents.size()); ++i) {
...
struct PuzzleInput
{
    ...
    QString solveTwo() const
    {
        ...
        for (const auto& packet : ordered) {
            if (packet.str == divider_packets_A.str or packet.str == divider_packets_B.str) {
                product *= index;
                if (first_found)
                    break;
                first_found = true;
