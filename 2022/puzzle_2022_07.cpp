...
public:
    ...
    const std::optional<Id>& parentDirectoryID() const { return m_parent_directory_ID; }
    const QMap<QString, Int>& files() const { return m_files; }
    const std::set<Id>& childrenDirectoriesIDs() const { return m_children_directories_IDs; }
    ...
    void addChild(Id ID) {
        m_children_directories_IDs.insert(ID);
    }
