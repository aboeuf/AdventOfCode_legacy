...
inline QStringList match(const QString& pattern, QStringList& input)
{
    ...
    if (not rx.exactMatch(input.front()))
        throw std::invalid_argument(QString("invalid line \"%1\"\nDid not match pattern \"%2\"")
                                    .arg(input.front())
                                    .arg(pattern)
                                    .toStdString());
    do {
...
public:
    ...
    {
        m_index = static_cast<std::size_t>(match("Monkey (\\d+):", lines)[1].toUInt());

        auto items_str = match("  Starting items: (.*)", lines)[1];
        items_str.remove(' ');
        const auto items = common::toUIntValues(items_str, ',');
        for (auto item : items)
        ...
        const auto is_addition = [&operation]() {
            ...
            if (operation[1] == "*")
                return false;
            throw std::invalid_argument(QString("invalid operator \"%1\"").arg(operation[1]).toStdString());
        }();
        if (operation[2] == "old") {
            if (is_addition)
            ...
            if (not ok)
                throw std::invalid_argument(QString("invalid operand \"%1\"").arg(operation[2]).toStdString());
            if (is_addition)
        ...
        m_monkey_if_true = static_cast<std::size_t>(match("    If true: throw to monkey (\\d+)", lines)[1].toUInt());
        m_monkey_if_false = static_cast<std::size_t>(match("    If false: throw to monkey (\\d+)", lines)[1].toUInt());
    }
    ...
    std::vector<ItemToThrow> applyTurn(bool relaxed)
    {
        ...
        while (not m_items.empty()) {
            ...
            if (relaxed)
                worry_level /= 3;
            items.emplace_back(worry_level,
                               worry_level % m_test == 0 ? m_monkey_if_true : m_monkey_if_false);
        }
        return items;
    }
...
public:
    ...
    QString solvePuzzle(uint nb_rounds, bool relaxed) {
        for (auto i = 0u; i < nb_rounds; ++i) {
            for (auto& monkey : m_monkeys) {
                ...
                for (const auto& item : items)
                    m_monkeys[item.throw_to].catchItem(relaxed ? item.worry_level : item.worry_level % m_mod);
            }
        }
        ...
        std::sort(std::begin(nb_inspections), std::end(nb_inspections), [](auto lhs, auto rhs) {
            return lhs > rhs;
        });
