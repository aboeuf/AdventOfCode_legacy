...
const auto adjacent_digits_rule = [](const int &nb_adjacent) -> Rule {
  return [&nb_adjacent](const QString &password) -> bool {
    int count = 1;
    for (auto it = std::next(password.begin());
         count != nb_adjacent && it != password.end(); ++it) {
      if (*it == *std::prev(it))
        ++count;
      else
        count = 1;
...
void Solver_2019_04_1::solve(const QString &input) {
  using namespace puzzle_2019_04;
  int min, max;
  if (!getRange(input, min, max)) {
    emit finished("FAILURE");
    return;
  }
  const std::vector<Rule> rules = {adjacent_digits_rule(2),
                                   non_decreasing_rule};
  emit finished(QString::number(nbValid(rules, min, max)));
}

void Solver_2019_04_2::solve(const QString &input) {
  using namespace puzzle_2019_04;
  int min, max;
  if (not getRange(input, min, max)) {
    emit finished("FAILURE");
    return;
  }
  const std::vector<Rule> rules = {adjacent_digits_rule_modified(2),
                                   non_decreasing_rule};
  emit finished(QString::number(nbValid(rules, min, max)));
}
