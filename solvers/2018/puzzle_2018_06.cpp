...
DisplayData VoronoiDiagram::getDisplayData() const {
  auto data = DisplayData{};

  const auto delta_x = m_xmax - m_xmin;
  const auto delta_y = m_ymax - m_ymin;
  data.rectangles.reserve(delta_x * delta_y + std::size(m_points) + 1);
  ...
  for (auto x = m_xmin - 25; x <= m_xmax + 25; ++x) {
    for (auto y = m_ymin - 25; y <= m_ymax + 25; ++y) {
      ...
      if (closests.size() == 1) {
        ...
        data.rectangles.emplace_back(QRectF{static_cast<double>(x) - 0.5,
                                            static_cast<double>(y) - 0.5, 1.0,
                                            1.0},
  ...
  for (const auto &pt : m_points) {
    ...
    data.rectangles.emplace_back(QRectF{x - 0.5, y - 0.5, 1.0, 1.0}, pen,
                                 QBrush{Qt::NoBrush});
  }
  data.rectangles.emplace_back(
      QRectF{static_cast<qreal>(m_xmin), static_cast<qreal>(m_ymin),
             static_cast<qreal>(delta_x), static_cast<qreal>(delta_y)},
...
void Solver_2018_06_1::solve(const QString &input) {
  ...
  emit display(diagram.getDisplayData());
