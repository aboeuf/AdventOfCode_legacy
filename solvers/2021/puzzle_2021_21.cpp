...
constexpr auto nb_per_position_increments =
    std::array<Int, 7>{1, 3, 6, 7, 6, 3, 1};
...
struct Player {
  ...
  Player(const Player &other)
      : m_position{other.m_position}, m_score{other.m_score} {}
...
public:
  DeterministicDiracDiceGame(const QString &input) {
    ...
    m_result = QString("%1").arg(
        std::min(m_players[0].m_score, m_players[1].m_score) * dice.nbRolls());
  }
      ...
      for (auto it = std::cbegin(states); it != std::cend(states); ++it) {
        ...
        for (int i = 0; i < 7; ++i) {
          ...
          if (next_score < 21) {
            ...
            next_state[passive_player_id].m_position =
                passive_player.m_position;
            next_state[passive_player_id].m_score = passive_player.m_score;
            auto &nb_states =
                new_states.insert(std::make_pair(next_state, Int(0)))
                    .first->second;
            nb_states += nb_spawn;
          } else
