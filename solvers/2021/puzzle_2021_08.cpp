...
public:
  Entry(const QString &input) {
    ...
    std::sort(std::begin(input_patterns), std::end(input_patterns),
    ...
    // Identify the 3 (only pattern of size 5 that contains both segments that
    // the 1 also contains)
    ...
    // Identify the 6 (only pattern of size 6 that contains only one of the
    // segments that the 1 also contains) Recover the translation of the 'c'
    // segment (i.e. the missing segment)
    ...
    // Identify the 5 (only remaing pattern of size 5 that do not contain the
    // 'c' segment)
    for (auto it = std::begin(size_5); it != std::end(size_5); ++it) {
    ...
    // Identify the translation of the 'g' segment (only segment that is in 2
    // but not in 3)
    for (const auto &chr : pattern_3)
      pattern_2.remove(chr);
    auto g_segment = pattern_2.at(0);

    // Identify the 9 (only remaining pattern of size 6 that do not contain the
    // 'g' segment)
    for (auto it = std::begin(size_6); it != std::end(size_6); ++it) {
  ...
  const uint &nbObviousInOutput() const { return m_nb_obvious_patterns; }
