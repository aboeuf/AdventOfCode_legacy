...
struct Bag {
  ...
  int getNbAncestors() const {
    ...
    return ancestors.contains(m_color) ? ancestors.size() - 1
                                       : ancestors.size();
  }
  ...
  int getDescendantsWeight(QMap<QString, Bag *> &descendants) const {
    ...
    for (const Edge *edge : m_out_edges)
      if (!descendants.contains(edge->m_tail->m_color))
        weight +=
            edge->m_value * edge->m_tail->getDescendantsWeight(descendants);
    return weight;
  }
  ...
  AllBags(const QString &input) {
    ...
    for (const QString &line : lines) {
      ...
      for (auto it = values.begin(); it != values.end();) {
        ...
        if (word.contains("bag") || word == "contain" || word == "no" ||
            word == "other")
          it = values.erase(it);
        else
          ++it;
      ...
      for (int i = 2; i + 2 < values.size(); i += 3)
        rules.back().m_content[values[i + 1] + "_" + values[i + 2]] =
            values[i].toInt();
...
void Solver_2020_07_1::solve(const QString &input) {
  using namespace puzzle_2020_07;
  AllBags bags(input);
  emit finished(QString::number(bags.getNbAncestors("shiny_gold")));
}

void Solver_2020_07_2::solve(const QString &input) {
  using namespace puzzle_2020_07;
  using namespace puzzle_2020_07;
  AllBags bags(input);
