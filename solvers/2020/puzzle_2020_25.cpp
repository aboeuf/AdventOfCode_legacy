...
void Solver_2020_25_1::solve(const QString &input) {
  ...
  Int encryption_key_from_card =
      transformSubject(public_keys.second, loop_sizes.first);
  Int encryption_key_from_door =
      transformSubject(public_keys.first, loop_sizes.second);
  if (encryption_key_from_card != encryption_key_from_door) {
    emit finished(QString("Different encryption keys:\nCard: %1\nDoor: %2")
                      .arg(encryption_key_from_card)
                      .arg(encryption_key_from_door));
    return;
  }
