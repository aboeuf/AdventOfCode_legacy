...
public:
  ...
  Entry(const QString& input) {
    auto rx = QRegExp{"^\\[([0-9]+)-([0-9]+)-([0-9]+) ([0-9]+):([0-9]+)\\] (.+)$"};
    if (not rx.exactMatch(input))
      return;
    m_timestamp = QDateTime{QDate{rx.cap(1).toInt(), rx.cap(2).toInt(), rx.cap(3).toInt()},
...
public:
  Shift(std::vector<Entry>& entries, std::size_t& index) {
    ...
    auto asleep = false;
    for(; index < entries.size() and entries[index].type() != Entry::BEGINS_SHIFT; ++index) {
  ...
  QString toString() const {
    auto res = QString{"%1-%2 #%3   "}.arg(m_date.month(), 2, 10, QChar('0')).arg(m_date.day(), 2, 10, QChar('0')).arg(m_guard_id);
    for (auto i = 0; i < 60; ++i)
      res += (m_minutes[i] ? "#" : ".");
    return res;
  }
...
{
  QString toString() const {
    return QString{"%1"}.arg(m_guard_id * m_minute);
  }
  uint m_guard_id;
  uint m_minute;
};
...
public:
  ...
  std::optional<Solution> strategy_1() const
  {
    ...
    if (not sleepiest_guard)
      return std::nullopt;
    return Solution{sleepiest_guard->m_id, sleepiest_guard->mostSleptMinute().first};
  }
