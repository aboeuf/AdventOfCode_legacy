  ...
  for (auto y : m_solvers.m_solvers.keys()) {
    for (auto d : m_solvers.m_solvers[y].keys()) {
      for (auto p : m_solvers.m_solvers[y][d].keys()) {
        connect(m_solvers.m_solvers[y][d][p],
                SIGNAL(display(const DisplayData &)), this,
                SLOT(updateDisplay(const DisplayData &)));
      }
    }
  }
...
void MainWindow::updateDisplay(const DisplayData &data) {
  m_display.updateData(data);
}
...
QString MainWindow::createDefault() {
  ...
  if (pro_file_out.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    out << "RESOURCES += gui/resources.qrc\n\n";
  ...
  if (cmake_file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    out << ")\n"
           "\n"
           "target_include_directories(aoc_solvers PUBLIC ${CMAKE_CURRENT_SOURCE_DIR}/..)\n"
           "target_link_libraries(aoc_solvers Qt5::Core Qt5::Gui Qt5::Widgets)\n"
           "set_target_properties(aoc_solvers PROPERTIES POSITION_INDEPENDENT_CODE ON)\n";
  } else {
    return "Error: cannot write in file solvers/CMakeLists.txt";
  }
...
BoardConf &MainWindow::getCurrentBoardConf(QString *ret_id) {
  ...
  return m_config.m_leaderboards.contains(id) ? m_config.m_leaderboards[id]
                                              : m_error_access;
}
