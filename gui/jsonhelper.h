...
template <> struct Type<qint64> {
  static QJsonValue::Type get() { return QJsonValue::Double; }
  static int cast(const QJsonValue &value) {
    return static_cast<qint64>(value.toInt());
  }
};
...
public:
  ...
  bool read(const QString &filepath, QJsonDocument &document) {
    ...
    if (!file.open(QFile::Text | QFile::ReadOnly)) {
      m_error =
          "Cannot open file\"" + QFileInfo(file).absoluteFilePath() + "\"";
      return false;
    }
  ...
  template <typename T> bool read(const QJsonValue &json, T &value) {
  ...
  bool read(const QJsonArray &array, Integer index, T &value) {
    ...
    if (i < 0 or i >= array.size()) {
      m_error =
          QString("Out of range index %1 (casted to %2)").arg(index).arg(i);
      return false;
    }
