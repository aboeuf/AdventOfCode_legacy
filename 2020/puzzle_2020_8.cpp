  ...
  ReturnStatus run()
  {
    while (m_instruction_pointer >= 0 &&
      ...
      if (I.m_operation == "acc") {
        ...
        QString("Unknown operation \"%1\"").arg(I.m_operation);
        return ERROR;
      }
    }
    if (m_instruction_pointer != nb_instructions) {
      QString("Bad instruction pointer %1 (%2 instructions)")
...
void Solver_2020_8_1::solve(const QString& input)
{
  using namespace puzzle_2020_8;
  Program prog(input);
  if (prog.run() != ERROR)
...
void Solver_2020_8_2::solve(const QString& input)
{
  using namespace puzzle_2020_8;
