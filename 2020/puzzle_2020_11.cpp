...
public:
  ...
  {
    if (new_rules)
      m_is_changing = m_is_empty ? nbOccupiedVisible() == 0 : nbOccupiedVisible() > 4;
    else
      m_is_changing = m_is_empty ? nbOccupiedAdjacent() == 0 : nbOccupiedAdjacent() > 3;
  }
...
public:
  SeatMap(const QString& input)
  {
    ...
    m_map = QVector<QVector<Cell*>>(nb_columns, QVector<Cell*>(nb_columns, nullptr));
    uint i = 0;
    for (const QString& line : lines) {
