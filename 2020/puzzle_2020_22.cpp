...
struct RecursiveCombatGame
{
  ...
  RecursiveCombatGame(const Deck& player_1,
                      const Deck& player_2,
                      int n1, int n2, Int depth) : m_depth{depth + 1}
  {
    for (auto card = player_1.cards().begin();
         m_player_1.cards().size() != n1 &&
         card != player_1.cards().end(); ++card)
      m_player_1 << *card;

    for (auto card = player_2.cards().begin();
         m_player_2.cards().size() != n2 &&
         card != player_2.cards().end(); ++card)
      m_player_2 << *card;
  }
