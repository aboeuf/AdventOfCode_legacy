...
{
  Transformation{"F0R0"},
  Transformation{"F0R90"},
  Transformation{"F0R180"},
  Transformation{"F0R270"},
  Transformation{"F1R0"},
  Transformation{"F1R90"},
  Transformation{"F1R180"},
  Transformation{"F1R270"}
};
...
public:
  ...
  void debug() const
  {
    ...
    {
      QString cand = t + ": ";
      const QMap<MatchDirection, QMap<Tile*, QString>>& candidates = m_candidates[t];
  ...
  QList<MatchDirection> match(const Tile& other,
                              const Transformation& t_this,
  ...
  void addCandidate(const MatchDirection& dir, Tile* tile,
                    const QPair<Transformation, Transformation> transformations)
  {
    if (tile)
      m_candidates[transformations.first.toString()][dir][tile] = transformations.second.toString();
  }
...
public:
  Image(const QString& input)
  {
    ...
    for (auto ti = m_tiles.begin(); ti != m_tiles.end(); ++ti)
      for (auto tj = std::next(ti); tj != m_tiles.end(); ++tj)
        ti->setCandidates(*tj);
    int asize = static_cast<int>(std::sqrt(static_cast<double>(m_tiles.size())));
  ...
  const QVector<QVector<QPair<Tile*, QString>>>& image() const { return m_image; }
  ...
  void debug() const
  {
    ...
    for (int i = 0; i < m_image.size(); ++i) {
      QString row;
      for (int j = 0; j < m_image[i].size(); ++j)
        row += " " + (m_image[i][j].first ? QString::number(m_image[i][j].first->index()) : QString("null"));
      qDebug() << row.toStdString().c_str();
    }
    qDebug() << "Image" << m_image.size();
    for (int i = 0; i < m_image.size(); ++i) {
      QString row;
      for (int j = 0; j < m_image[i].size(); ++j)
        row += " " + (m_image[i][j].first ? m_image[i][j].second : QString("null"));
      qDebug() << row.toStdString().c_str();
    }
  }
