...
struct Ship
{
  ...
  {
    return static_cast<uint>(std::abs(m_position.x()) + std::abs(m_position.y()));
  }
...
{
  North(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
...
{
  South(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    if (m_new_system)
      ship.m_waypoint += static_cast<int>(m_value) * QPoint{0, -1};
...
{
  East(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    if (m_new_system)
...
{
  West(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    if (m_new_system)
      ship.m_waypoint += static_cast<int>(m_value) * QPoint{-1, 0};
...
{
  Left(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    ...
    for (uint i = 0; i < nb_steps; ++i)
      if (m_new_system)
        ...
        ship.m_orientation = QPoint{-ship.m_orientation.y(), ship.m_orientation.x()};
  }
};

struct Right : public NavigationInstruction
{
  Right(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    uint nb_steps = m_value / 90;
    for (uint i = 0; i < nb_steps; ++i)
...
{
  Forward(uint value, bool new_system) : NavigationInstruction{value, new_system} {}
  void apply(Ship& ship)
  {
    if (m_new_system)
