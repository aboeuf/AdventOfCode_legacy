...
Int toInt(const QString& str, bool& ok)
{
  return str.toULongLong(&ok, 2);
}

QString fromInt(Int n)
{
  return QString("%1").arg(n, 36, 2, QChar('0'));
}
...
{
  WriteV1(Int address, Int value) : Instruction(), m_address{address}, m_value{value} {}
  void runOn(Emulator& emulator) const override
  {
...
{
  WriteV2(Int address, Int value) : Instruction(), m_address{address}, m_value{value} {}
  void runOn(Emulator& emulator) const override
  {
    ...
    if (address.back() == '0')
      ...
      addresses << 0 << 1;
    Int pow = 2;
    for (auto it_char = std::next(address.rbegin()); it_char != address.rend(); ++it_char) {
      if (*it_char == 'X') {
