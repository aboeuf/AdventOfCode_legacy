...
int nbOnes(const QStringList& diagnostic, int bit_index)
{
  ...
  for (const auto& binary_entry : diagnostic) {
    if (bit_index < binary_entry.size() and binary_entry[bit_index] == QChar('1'))
      ++count;
  }
  return count;
}
...
struct Gamma : public QChar
{
  Gamma(const QStringList& diagnostic, int bit_index)
    : QChar{diagnostic.size() > 2 * nbOnes(diagnostic, bit_index) ? '0' : '1'} {}
};

struct Epsilon : public QChar
{
  Epsilon(const QStringList& diagnostic, int bit_index)
    : QChar{diagnostic.size() > 2 * nbOnes(diagnostic, bit_index) ? '1' : '0'} {}
};

template <typename ExpectedChar>
QString getRating(QStringList diagnostic)
{
  if (diagnostic.empty())
    return QString{};
  const auto nb_bits = diagnostic.front().size();
  for (auto current_bit = 0; diagnostic.size() > 1 and current_bit < nb_bits; ++current_bit) {
...
{
public:
  BinaryDiagnostic(const QString& input)
  {
    m_raw = common::splitLines(input);
  }
  ...
  QString lifeSupportRating() const
  {
    ...
    if (not conversion_ok)
      return "Cannot convert oxygen_generator_rating: " + oxygen_generator_rating;
