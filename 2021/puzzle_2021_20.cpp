...
private:
  bool isLit(Int x, Int y) const
  {
    if (m_x_min <= x and x <= m_x_max and m_y_min <= y and y <= m_y_max)
      return m_different_than_outside.find(Pixel(x, y)) == std::end(m_different_than_outside) ? m_outside_is_lit : not m_outside_is_lit;
  ...
  void enhanceOnce()
  {
    if (m_different_than_outside.empty())
      return;
    auto next_outside_is_lit = m_enhancement_algorithm[m_outside_is_lit ? 511 : 0];
