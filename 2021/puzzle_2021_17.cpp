...
struct Probe
{
  Probe(const Target& target, int vx, int vy) :
    ...
    m_max_height{0}
  {
    while (not isIn(target) and
           not hasMissed(target)) {
      advanceOneStep();
    }
  }
  ...
  {
    return target.m_x_min <= m_x and
        m_x <= target.m_x_max and
        target.m_y_min <= m_y and
        m_y <= target.m_y_max;
  }
...
{
public:
  ShotComputer(const QString& target) :
    m_target{target}
  {
  }
