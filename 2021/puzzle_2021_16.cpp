  ...
  Int value() const {
    ...
    case 5u:
      return (m_sub_packets.size() > 1 and
              m_sub_packets[0]->value() > m_sub_packets[1]->value()) ? 1u : 0u;
    case 6u:
      return (m_sub_packets.size() > 1 and
              m_sub_packets[0]->value() < m_sub_packets[1]->value()) ? 1u : 0u;
    case 7u:
...
public:
  Transmission(const QString& input)
  {
    ...
    for (const auto& chr : input) {
      if (chr != '\n')
        binary += QString("%1").arg(QString(chr).toInt(nullptr, 16), 4, 2, QChar('0'));
    }
    m_packet = Packet(binary);
  }

  QString versionSum() const { return QString("%1").arg(m_packet.versionSum()); }
