...
public:
  ...
  uint mark(uint value)
  {
    ...
    for (auto i = 0; i < 5; ++i) {
      for (auto j = 0; j < 5; ++j) {
        if (not m_grid[i][j].marked and
            m_grid[i][j].value == value) {
...
public:
  BingoGame(const QString& input)
  {
    ...
    m_called_numbers = common::toUIntValues(*it, QChar(','));
    auto current_row = 5u;
    ++it;
    for (; it != std::end(lines); ++it) {
      if (not it->isEmpty()) {
        ...
        m_boards.back().setRow(current_row, common::toUIntValues(*it, QChar(' ')));
        ++current_row;
      }
    }
  }
