  ...
  RiskMap(const QString& input, Int nb_tiles_per_row)
  {
    ...
    for (const auto& line : lines) {
      ...
      for (const auto& chr : line)
        risk_levels.back().emplace_back(static_cast<Int>(QString("%1").arg(chr).toInt() - 1));
    }
    ...
    const auto add_edge = [&weights, &graph, &get_risk_level, &vertices](auto pi, auto pj, auto i, auto j) {
      weights[boost::add_edge(vertices[pi][pj], vertices[i][j], graph).first] = get_risk_level(i, j);
    };
    for (auto i = 0; i < N; ++i) {
      for (auto j = 0; j < N; ++j) {
        if (i > 0) {
    ...
    boost::dijkstra_shortest_paths(graph, vertices.front().front(), boost::distance_map(&dist_map[0]));
